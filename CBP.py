...
# setting global variables - this allows the processes to tell eachother when a specific platform is free/emptys
    ...
    server6 = Server(env,1,6)
    server7 = Server(env,1,7)
    ...
    if(server1ProcessingTime<= threshold):
        ...
        car.remove_car()
    elif(server2ProcessingTime<=threshold):
        #pass to server 2
        car.move_car(-105, 0)
        server2.process(car)
        ...
        car.remove_car()
    elif(server3ProcessingTime<=threshold):
        #pass to server 3
        car.move_car(-110, 0)
        yield env.timeout(run_time/3)
        yield env.timeout(run_time/3)
        ...
        car.remove_car()
    elif(server4ProcessingTime<=threshold):
        #pass to server 4
        car.move_car(-115, 0)
        yield env.timeout(run_time/3)
        yield env.timeout(run_time/3)
    ...
    minimumDelay = min(server1.processingTime, server2.processingTime, server3.processingTime, server4.processingTime, server5.processingTime, server6.processingTime,server7.processingTime)
    ...
    ax.plot(processingTime, label='CBT model, C=5')
