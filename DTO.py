...
# setting global variables - this allows the processes to tell eachother when a specific platform is free/emptys
    ...
    if(server1ProcessingTime<= 0.22):
        ...
        car.remove_car()
    elif(server2ProcessingTime<=0.3):
        #pass to server 2
        car.move_car(-105, 0)
        server2.process(car)
        ...
        car.remove_car()
    elif(server3ProcessingTime<=0.35):
        #pass to server 3
        car.move_car(-110, 0)
        yield env.timeout(run_time/3)
        yield env.timeout(run_time/3)
        ...
        car.remove_car()
    elif(server4ProcessingTime<=0.4):
        #pass to server 4
        car.move_car(-115, 0)
        yield env.timeout(run_time/3)
        yield env.timeout(run_time/3)
    ...
    ax.plot(processingTime, label='DTO model')
