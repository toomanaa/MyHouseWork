...
# setting global variables - this allows the processes to tell eachother when a specific platform is free/emptys
    ...
    car = Car(canvas, 800,165,750,185, name)
        ...
        #car.remove_car()
    if(server2ProcessingTime<=threshold):
        #pass to server 2
        car.move_car(-105, 0)
        server2.process(car)
        ...
        d = np.column_stack([car_id, server3.processingTime, server3.id])
        car.remove_car()
    ...
    ax.plot(processingTime, label='Odds model, Threshold=50')
