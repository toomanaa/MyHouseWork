...
processingTime = []
optimalDelay = []
simulationInformation = [("Car Id", "Requesting Time", "Accessing Time", "Finishing Time","serverProcessingTime","processDuration", "Waiting time", "NumofUsers","ServerId")]
n = 0

# set up general parameters
RANDOM_SEED = 45
T_INTER = 10 # Arrival headway: create a car on average every T_INTER seconds, setting this to "1" is a reasonable approximation to "as fast as possible" without slowing down the simulation
...
# setting global variables - this allows the processes to tell eachother when a specific platform is free/emptys
    ...
    mylist = []
    mylist= bernoulli.rvs(size=5,p=0.8)
    print(mylist)

    for index in mylist:
        if(mylist[index]==1):
            serverSelection = index
    ...
    ax.plot(processingTime, label='P-model model')
