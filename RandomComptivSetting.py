...
def car(env, name, serverSelection):
...
# setting global variables - this allows the processes to tell eachother when a specific platform is free/emptys
    ...
    if(serverSelection==1):
        with server1.server.request() as request:
            ...
            car.remove_car()
            server = server1.id
            waiting_time = gettingTime - requestingTime
            waiting_time2.append(gettingTime - requestingTime)
            simulationInformation.append((serverSelection,car_id, requestingTime, gettingTime, env.now, serverProcessingTime ,processDuration ,waiting_time, serverStatus, server))
    ...
    print("====================")
    print("mean")
    print(np.mean(waiting_time2))
    if show_animation == True and hide_plots == False:
        a1.cla()
        a1.set_xlabel("Time")
        a1.set_ylabel("Processing Time")
        a1.plot(waiting_time2)
...
def setup(env, num_servers, t_inter):
    ...
    for i in range(1):
        env.process(car(env, 'Car %d' % i, random.randint(1,4)))

    # Create more cars while the simulation is running
    while True:
        yield env.timeout(arrival_interval(t_inter))
        i += 1
        env.process(car(env, i,  random.randint(2,4)))
