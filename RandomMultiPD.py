...
class Server(object):
    ...
    def __init__(self, env, num_vm, id, mu, sd):
    #def __init__(self, env, num_vm, id):
        ...
        self.mu = mu
        self.sd = sd
        self.processingTime = np.random.normal(self.mu, self.sd, 1)
        #self.processingTime= np.random.uniform(10, 100, 1)
...
# setting global variables - this allows the processes to tell eachother when a specific platform is free/emptys
    ...
    server1 = Server(env,1,1, random.randint(20,100), random.randint(1,5))
    server2 = Server(env,1,1, random.randint(20,100), random.randint(1,5))
    server3 = Server(env,1,1, random.randint(20,100), random.randint(1,5))
    server4 = Server(env,1,1, random.randint(20,100), random.randint(1,5))
    server5 = Server(env,1,1, random.randint(20,100), random.randint(1,5))
    ...
    print("we are offloading at server ")
    print(rd)
    if(rd == 1):
        #pass to server 1
        yield env.timeout(run_time/3)
        ...
        car.move_car(-100, 0)
        server1.process(car)
        processingTime.append(server1ProcessingTime)
        simulationInformation.append((car_id, server1ProcessingTime, server1.id))
        #car.remove_car()
    elif(rd == 2):
        #pass to server 2
        car.move_car(-105, 0)
        server2.process(car)
        processingTime.append(server2ProcessingTime)
        simulationInformation.append((car_id, server2ProcessingTime, server2.id))
        car.remove_car()
    elif(rd == 3):
        #pass to server 3
        car.move_car(-110, 0)
        ...
        simulationInformation.append((car_id, server3ProcessingTime, server3.id))
        car.remove_car()
    elif(rd == 4):
        #pass to server 4
        car.move_car(-115, 0)
        ...
        simulationInformation.append((car_id, server4ProcessingTime, server4.id))
        car.remove_car()
    else:
         #pass to server 5
        car.move_car(-250, 0)
        ...
        simulationInformation.append((car_id, server5ProcessingTime,server5.id))
        car.remove_car()
    ...
    ax.plot(processingTime, label='Random model')
    ...
    if show_animation == True and hide_plots == False:
        ...
        plt.savefig('RandomResultMultiPD.eps')
...
with open("out.csv", mode="w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow('simulationInformation')
